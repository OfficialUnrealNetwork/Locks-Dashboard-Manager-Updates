...
export function validateOptsIgnore(ignore) {
    if (ignore) {
...
async function signApplication(opts, identity) {
     ...
     */
    const filesWithSameArgsByDepth = new Map();
    for (const filePath of [...children, opts.app]) {
        if (shouldIgnoreFilePath(filePath)) {
            debugLog('Skipped... ' + filePath);
            continue;
        }
        const perFileOptions = await mergeOptionsForFile(opts.optionsForFile ? opts.optionsForFile(filePath, { platform: opts.platform }) : null, defaultOptionsForFile(filePath, opts.platform));
        ...
        if (opts.batchCodesignCalls) {
            ...
            const filesWithSameArgsMap = filesWithSameArgsByDepth.get(fileDepth) ?? new Map();
            const fileWithSameArgsMapKey = JSON.stringify(perFileArgs);
            filesWithSameArgsMap.set(fileWithSameArgsMapKey, (filesWithSameArgsMap.get(fileWithSameArgsMapKey) ?? []).concat(filePath));
