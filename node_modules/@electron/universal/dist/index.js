...
import { AsarMode, detectAsarMode, detectEntrypointModule, EntrypointModule, isUniversalMachO, mergeASARs, resolveShimModule, } from './asar-utils.js';
import { AppFileType, compareDirectories, fsMove, getAllAppFiles, matchGlob, readMachOHeader, } from './file-utils.js';
import { sha } from './sha.js';
import { d } from './debug.js';
import { computeIntegrityData } from './integrity.js';
const execFile = promisify(execFileCb);
const dupedFiles = (files) => files.filter((f) => f.type !== AppFileType.SNAPSHOT &&
    ...
    try {
        ...
        for (const file of multiArchFiles.filter((f) => f.type === AppFileType.PLAIN)) {
            ...
            if (x64Sha !== arm64Sha) {
                ...
                throw new Error(`Expected all plain files (those that are not Mach-O binaries, V8 snapshots, Info.plist files, or ASAR archives) to have identical SHAs when creating a universal build but "${file.relativePath}" did not`);
            }
        }
        ...
        for (const machOFile of multiArchFiles.filter((f) => f.type === AppFileType.MACHO)) {
            ...
            const second = await fs.promises.realpath(path.resolve(opts.arm64AppPath, machOFile.relativePath));
        ...
        if (x64AsarMode === AsarMode.NO_ASAR) {
            ...
            const diffSet = await compareDirectories(path.resolve(tmpApp, 'Contents', 'Resources', 'app'), path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app'));
            ...
            if (nonMergedDifferences.length > 0) {
                ...
                const arm64Pj = JSON.parse(await fs.promises.readFile(path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app', 'package.json'), 'utf8'));
                const shimModule = resolveShimModule(detectEntrypointModule(pj), detectEntrypointModule(arm64Pj));
                if (shimModule === EntrypointModule.ESM) {
                    await fs.promises.cp(path.resolve(import.meta.dirname, '..', 'entry-asar', 'esm', 'no-asar.mjs'), path.resolve(entryAsar, 'index.mjs'));
                    pj.main = 'index.mjs';
                }
                else {
        ...
        if (x64AsarMode === AsarMode.HAS_ASAR && opts.mergeASARs) {
            ...
            if (x64AsarSha !== arm64AsarSha) {
                ...
                if (fs.existsSync(x64Unpacked)) {
                    await fsMove(x64Unpacked, path.resolve(tmpApp, 'Contents', 'Resources', 'app-x64.asar.unpacked'));
                }
                const arm64AsarPath = path.resolve(tmpApp, 'Contents', 'Resources', 'app-arm64.asar');
                await fs.promises.cp(path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar'), arm64AsarPath, { force: true, recursive: true, verbatimSymlinks: true });
                const arm64Unpacked = path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar.unpacked');
                if (fs.existsSync(arm64Unpacked)) {
                    await fs.promises.cp(arm64Unpacked, path.resolve(tmpApp, 'Contents', 'Resources', 'app-arm64.asar.unpacked'), { force: true, recursive: true, verbatimSymlinks: true });
                }
                const entryAsar = path.resolve(tmpDir, 'entry-asar');
                await fs.promises.mkdir(entryAsar, { recursive: true });
                ...
                const arm64Pj = JSON.parse(asar
                    .extractFile(path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar'), 'package.json')
                    .toString('utf8'));
                const shimModule = resolveShimModule(detectEntrypointModule(pj), detectEntrypointModule(arm64Pj));
                if (shimModule === EntrypointModule.ESM) {
                    await fs.promises.cp(path.resolve(import.meta.dirname, '..', 'entry-asar', 'esm', 'has-asar.mjs'), path.resolve(entryAsar, 'index.mjs'));
                    pj.main = 'index.mjs';
                }
                else {
                    await fs.promises.cp(path.resolve(import.meta.dirname, '..', 'entry-asar', 'has-asar.js'), path.resolve(entryAsar, 'index.js'));
                    pj.main = 'index.js';
                }
                await fs.promises.writeFile(path.resolve(entryAsar, 'package.json'), JSON.stringify(pj) + '\n', 'utf8');
        ...
        for (const plistFile of plistFiles) {
            ...
            const { ElectronAsarIntegrity: x64Integrity, ...x64Plist } = plist.parse(await fs.promises.readFile(x64PlistPath, 'utf8'));
            const { ElectronAsarIntegrity: arm64Integrity, ...arm64Plist } = plist.parse(await fs.promises.readFile(arm64PlistPath, 'utf8'));
            ...
            const injectAsarIntegrity = !opts.infoPlistsToIgnore || matchGlob(plistFile.relativePath, opts.infoPlistsToIgnore);
            const mergedPlist = injectAsarIntegrity
                ? { ...x64Plist, ElectronAsarIntegrity: generatedIntegrity }
                : { ...x64Plist };
            await fs.promises.writeFile(path.resolve(tmpApp, plistFile.relativePath), plist.build(mergedPlist));
        }
        for (const snapshotsFile of arm64Files.filter((f) => f.type === AppFileType.SNAPSHOT)) {
            ...
            await fs.promises.cp(path.resolve(opts.arm64AppPath, snapshotsFile.relativePath), path.resolve(tmpApp, snapshotsFile.relativePath));
        }
