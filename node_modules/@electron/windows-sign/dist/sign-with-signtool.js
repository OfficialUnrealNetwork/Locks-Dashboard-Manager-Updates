 ...
 */
export function parseSignWithParams(input) {
    const tokens = [];
    let current = '';
    let inToken = false;
    let inQuotes = false;
...
export function getSigntoolArgs(options) {
    ...
    const hasUserFlag = (flag) => extraArgs.includes(flag);
...
export async function signWithSignTool(options) {
    ...
    const timestampServer = options.timestampServer ||
        process.env.WINDOWS_TIMESTAMP_SERVER ||
        'http://timestamp.digicert.com';
    const signToolPath = options.signToolPath ||
